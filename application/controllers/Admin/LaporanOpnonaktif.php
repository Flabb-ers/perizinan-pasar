...
class LaporanOPnonaktif extends CI_Controller {
    ...
    public function laporansemua(){
            $pengajuan = $this->M_op->tampilJoinNonActiveOP()->result();
        ...
        $this->template->load('pages/index','Admin/v_laporan/readOPNonaktif', $data); 
    }
...
public function filterPengajuan()
    {
        ...
        if ($id_pasar !== 'Pilih Pasar') {
            $data['pengajuan'] = $this->M_op->filterNonAktif($id_pasar);
        }else{
            $data['pengajuan'] = $this->M_op->filterByDateNonaktif();
        }
    ...
    public function exportLaporanPasar()        
    {
        ...
        if ($id_pasar !== 'Pilih Pasar') {
            $dataPengajuan = $this->M_op->filterNonAktif($id_pasar);
        }else{
            $dataPengajuan = $this->M_op->filterByDateNonaktif();
        }
        ...
        header("Content-Disposition: attachment; filename=Data Objek Pajak.xls");
