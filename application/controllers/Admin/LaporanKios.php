...
class LaporanKios extends CI_Controller {
    ...
    public function laporansemua(){
            $pengajuan = $this->M_kios->read()->result();
        ...
        $this->template->load('pages/index','Admin/v_laporan/readKios', $data); 
    }
...
public function filterPengajuan()
    {
        ...
        if ($id_pasar !== 'Pilih Pasar') {
            $data['pengajuan'] = $this->M_kios->filterByDateDinas($id_pasar);
        }else{
            $data['pengajuan'] = $this->M_kios->filterByDate();
        }
    ...
    public function exportLaporanPasar()        
    {
        ...
        if ($id_pasar !== 'Pilih Pasar') {
            $dataPengajuan = $this->M_kios->filterByDateDinas($id_pasar);
        }else{
            $dataPengajuan = $this->M_kios->filterByDate();
        }
